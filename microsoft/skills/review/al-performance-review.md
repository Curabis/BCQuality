...
- Worklist `al-methods-limited-during-write-transactions.md` when `Page.RunModal`, or `Report.RunModal`/`XmlPort.RunModal` without `false` as the request-page argument and without `UseRequestPage(false)`, follows an `Insert`, `Modify`, or `Delete` in the same trigger or procedure with no intervening `Commit`. Do not worklist it from a `RunModal` that precedes every write, or from a report or XMLport run with its request page suppressed. A `Codeunit.Run` whose return value is used in that position belongs to `codeunit-run-requires-prior-commit-inside-transaction.md`.
